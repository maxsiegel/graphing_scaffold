var jsPsychCustomGraphRise = (function (jspsych) {
  'use strict';
  ...
  const info = {
    ...
    parameters: {
       ...
       * For this specific plugin, the image is cut off at the graph axis and rises as the user drags the image up. */
  ...
  class GraphRisePlugin {
    ...
    async trial(display_element, trial) {
      ...
      bar_div.style.marginTop = Math.round(max_height * (1 - start_ratio)) + "px";
      ...
      const mousedownevent = (e) => {
        ...
        cy = parseInt(bar_div.style.marginTop);
      };
      ...
      const resizeevent = (e) => {
        if (dragging) {
          ...
          const dy = pointer_y - origin_y; // flipped because aligned at bottom
          let new_y = cy + dy;
          new_y = Math.max(new_y, 0);
          new_y = Math.min(new_y, max_height * 0.9);
          bar_div.style.marginTop = Math.round(new_y) + "px";
        }
      };
