var jsPsychCustomGraphGrow = (function (jspsych) {
  'use strict';
  ...
  const info = {
    ...
    parameters: {
       ...
       * For this specific plugin, the image is anchored at the graph axis and grows as the user drags the image. */
  ...
  class GraphGrowPlugin {
    ...
    async trial(display_element, trial) {
      ...
      // customize html here
      const custom_bar_style = "align-items: end;";
      const custom_img_style = 
        "cursor: nwse-resize;" + 
      ...
      bar_div.style.height = Math.round(max_height * start_ratio) + "px";
      let dragging = false;
      let origin_y, cy;
      ...
      const mousedownevent = (e) => {
        e.preventDefault();
        dragging = true;
        origin_y = e.pageY || e.targetTouches[0].pageY;
        cy = parseInt(bar_div.style.height);
      };
      ...
      const resizeevent = (e) => {
        if (dragging) {
          ...
          const dy = origin_y - pointer_y; // flipped because aligned at bottom
          const new_y = Math.min(Math.max(max_height / 10, cy + dy), max_height);
          bar_div.style.height = Math.round(new_y) + "px";
        }
      };
      ...
      display_element.querySelector("#jspsych-resize-div").addEventListener("mousedown", mousedownevent);
      display_element.querySelector("#jspsych-resize-div").addEventListener("touchstart", mousedownevent);
      ...
      const end_trial = () => {
        ...
        const final_height_px = bar_div.offsetHeight;
        var trial_data = {
